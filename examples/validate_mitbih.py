...
from src.io.mitbih import load_mitbih_record, list_mitbih_records, get_record_info
from src.analysis.validation import calculate_metrics, aggregate_results, ValidationResult
...
def validate_single_record(record_name: str, verbose: bool = True) -> ValidationResult:
    ...
    if verbose:
        ...
        print(f"  Duration: {info['duration_minutes']:.1f} min, Ground truth beats: {len(ground_truth)}")
        print(f"  Detected: {len(lead.r_peaks)}, TP: {result.true_positives}, FP: {result.false_positives}, FN: {result.false_negatives}")
        print(f"  Se: {result.sensitivity*100:.2f}%, +P: {result.positive_predictivity*100:.2f}%, F1: {result.f1_score:.4f}")
...
def validate_all_records(verbose: bool = True) -> dict:
    ...
    print(f"Gross Positive Predictivity: {agg['gross_positive_predictivity']*100:.2f}%")
    print(f"Gross F1 Score: {agg['gross_f1_score']:.4f}")
    print(f"\nAverage Sensitivity: {agg['avg_sensitivity']*100:.2f}%")
    print(f"Average Positive Predictivity: {agg['avg_positive_predictivity']*100:.2f}%")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Validate R peak detection against MIT-BIH')
    parser.add_argument('--record', '-r', type=str, help='Single record to validate (e.g., 100)')
    parser.add_argument('--quiet', '-q', action='store_true', help='Suppress per-record output')
